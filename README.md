...
如果你希望我把本地 `Figures/Theme/` 下的 logo、Cover PDF 等也一并通过 Git LFS 上传，我可以继续：
- 我可以为你把这些文件添加到仓库（会在远端显示为 LFS 指针）。
- 或者我可以保留占位符 `Figures/.gitkeep`，由你在本地执行上述 LFS 步骤与推送（更安全、你可检查授权与图像隐私）。

告诉我你更倾向哪种方式：我来继续自动上传（选项 1），或只留下说明让你在本地推送（选项 2）。